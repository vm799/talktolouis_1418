...
/**
 * Get a custom token source that also extracts userId for EventLog
 * @param onUserIdChange - Callback when userId is extracted from token response
 * @returns A token source that returns the token part to LiveKit
 */
export function getTokenSourceWithUserIdExtractor(onUserIdChange: (userId: string) => void) {
  return TokenSource.custom(async () => {
    try {
